package com.java.algorithms.io;
...
public class BufferedReader {
    ...
    public int peek(int n) throws IOException {
        ...
        if (n >= available) {
            throw new IOException(String.format("Out of range, available %d, but trying with %d", available, n));
        }
        pushRefreshData();

        if (n >= bufferSize) {
            throw new IllegalAccessError(String.format("Cannot peek %s, maximum upto %s (Buffer Limit)", n, bufferSize));
        }
        return buffer[n];
    }
